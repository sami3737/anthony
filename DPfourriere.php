<?php
	...
	if(isset($UserInfo[0]) && (isset($UserInfo[0]['job']) && $UserInfo[0]['job'] == 'depanneur' || $UserInfo[0]['admin'] == 1))
	{
		...
		{
			$db->query("INSERT INTO `facturefourriere` (`agent`, `client`, `paid`, `plaque`, `horodateur`, `prix`, `intervention`, `encaissement`) VALUES (:agent, :client, :paid, :plaque, :horo, :prix, :inter, :encaissement)", Array('agent' => $UserInfo[0]['name'], 'client' => $_POST['cname'], 'paid' => $_POST['pay'] == 'oui' ? 1 : 0, 'plaque' => $_POST['imma'], 'horo' => $_POST['horo'], 'prix' => $_POST['price'], 'inter' => $_POST['dataInsert'], 'encaissement' => $_POST['ename']));
                ...
                <div class="card-body">
                    ...
                    <form method="POST" action="">
                        <div class="row row-space">
							...
							<div class="col-2">
								<div class="input-group">
									<input class="input--style-2" type="text" placeholder="Nom du client" name="cname">
								</div>
							</div>
                        </div>
						<div class="row row-space">
                        ...
                        <div class="row row-space">
                            <div class="col-2">
								<div class="check full">
									<input hidden="hidden" class="input--style-2" id="dataInsert" type="input" name="dataInsert" value="">
										<?php
										$options = $db->query("Select * FROM interventionfourriere", Array());
										for($i = 0; $i < count($options); $i++)
											echo '<input data-index="'.$options[$i]['id'].'" type="radio" name="inter" value="'.$options[$i]['price'].'">'.$options[$i]['name'].'<br />';
										?>
							...
							<div class="col-2">
								<div class="input-group">
									<input class="input--style-2" type="text" placeholder="Nom de l'encaissement" name="ename">
								</div>
							</div>
                        </div>
                        <div class="p-t-30">
